...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        {/if}
        {if $href}<a href={$href}{if and( is_set( $link_class ), $link_class )} class="{$link_class}"{/if}{if and( is_set( $link_id ), $link_id )} id="{$link_id}"{/if}{if $target} target="{$target}"{/if}{if and( is_set( $link_title ), $link_title )} id="{$link_title|wash}"{/if}>
	...
	{else}
	{def $youtube_id = $attribute.content.alternative_text|explode( '=' )|extract_right( 1 ) }
	<iframe width="{$image.width}" height="{$image.height}" src="//www.youtube.com/embed/{$youtube_id.0|wash( xhtml )}" frameborder="0" allowfullscreen></iframe>
	{undef $youtube_id}
	{/if}
    {/if}
